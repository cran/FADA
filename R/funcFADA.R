...
FA = function(dta,test=NULL,nbf=NULL,nfolds=10,grouped=FALSE,plot.diagnostic=FALSE,min.err = 0.001,verbose=TRUE){
  ...
  nbfactors = function(dta,maxnbfactors=12,diagnostic.plot=plot.diagnostic,min.err=1e-03){
    dig = 2
    m = ncol(dta)
    n = nrow(dta)
    falist = vector(length=maxnbfactors+1,"list")
    falist[[1]] = list(B=matrix(0,ncol=1,nrow=m))
    falist[-1] = lapply(1:maxnbfactors,emfa,data=dta,min.err=min.err)
  ...
  ifa = function(Psi,B) {
    ...
    G = diag(q)+ crossprod(B,sweep(B,1,Phi,"*"))
    diag(Phi) - sweep(B,1,Phi,"*") %*% tcrossprod(solve(G),sweep(B,1,Phi,"*"))
  }
  
emfa = function (data, nbf, EM=TRUE,min.err = 1e-06) {
    n = nrow(data)
    m = ncol(data)
    S = var(data)
    ...
    if ((n>m)&&(m<=200)&&(m>=3)&&(!EM)) {
      ...
      if (nbf > 0) {
        ...
        Factors = tcrossprod(tcrossprod(cdata,sB) ,G)
      }   
    }
    if ((n<=m)||(m>200)||(m<=2)|EM) {
      if (nbf == 0) {
      ...
      if (nbf > 0) {
        ...
        evectors = eig$u[, 1:nbf,drop=FALSE]
        evalues = eig$d^2
        if (nbf > 1) 
          B = sweep(evectors[, 1:nbf,drop=FALSE] ,2,sqrt(evalues[1:nbf]),"*")
        if (nbf == 1) 
          B = matrix(evectors, nrow = m, ncol = 1) * sqrt(evalues[1])
        Psi = diag(S) - apply(B^2, 1, sum)
        crit = 1
        # cette partie a recoder en C
        while (crit > min.err) {
          iS = ifa(Psi,B)
          Cyz = crossprod(S,crossprod(iS,B))
          Czz = crossprod(B,iS) %*% Cyz+diag(nbf)-crossprod(B,iS)%*%B
          Bnew = tcrossprod(Cyz,solve(Czz))
          Psinew = diag(S)-diag(tcrossprod(Bnew,Cyz))
  ...
  if (is.null(nbf)) { nbf = nbfactors(scale(cdta,center=FALSE,scale=TRUE))$optimalnbfactors }
  ...
  if (nbf == 0) { if (is.null(test)) {dta.tmp=dta$x} else {dta.tmp = NULL}
    ...
    while (eps>min.err) {
      fa = emfa(cdta,nbf=nbf,min.err=1e-06)
    ...
    coefpx = LassoML(dta=fadta,test=fatest) 
    faproba.test = coefpx$proba.test
    fapredict.test = coefpx$predict.test
    }
...
FADA.tmp = function (faobject, method = c("glmnet", "sda", "sparseLDA"), 
    sda.method = c("lfdr", "HC"), stop.par , lambda, lambda.var, 
    lambda.freqs, diagonal , alpha ) 
{
    ...
    if (method == "sda") {
        ...
        pred = sda::predict.sda(out, matrix(fatest[, selected], 
            nrow = nrow(fatest), byrow = FALSE), verbose = FALSE)
...
cv.FADA = function (dta, nfold.cv, nbf.cv, method = c("glmnet", 
    ...
    diagonal = FALSE, alpha = 0.1) 
{
    ...
    dta.cv = list()
    test.cv = list()
    cv.fa = sapply(1:length(fold), function(j) {
        print(paste("CV - Fold", j, sep = " "))
        dta.cv$x = dta$x[-fold[[j]], ]
        dta.cv$y = dta$y[-fold[[j]]]
        test.cv$x = matrix(dta$x[fold[[j]], ], ncol = p)
        fa = FA(dta.cv, test.cv, nbf = nbf.cv, nfolds = nfolds, 
            verbose = FALSE)
        test.cv$y = dta$y[fold[[j]]]
        fada = FADA.tmp(fa, method = method, sda.method = sda.method, 
